...
import { AddStdtourplanComponent } from './add-stdtourplan/add-stdtourplan.component';

@Component({
  selector: 'app-std-tourplan',
  templateUrl: './std-tourplan.component.html',
  styleUrls: ['./std-tourplan.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: fuseAnimations
})
export class StdTourplanComponent implements OnInit {
  ...
  constructor(private _fuseSidebarService: FuseSidebarService,
    public _StdTourplanService: StdTourplanService,
  ...
  ngOnInit(): void {
    ...
    var D_data = {
      "MRId": this._StdTourplanService.myFilterform.get("MrId").value || "0",
      "PlaneYear": this._StdTourplanService.myFilterform.get("PlaneYear").value || "0",
      "PlaneMonth": this._StdTourplanService.myFilterform.get("PlaneMonth").value || "0",

    }
    console.log('itinit');
    this.dData1 = D_data;
    this._StdTourplanService.getTourplaneDetails(D_data).subscribe(Visit => {
  ...
  getMRList() {
debugger;
    this._StdTourplanService.getMRCombo().subscribe(data => {
      this.MRList = data;
      this.filteredMR.next(this.MRList.slice());
  ...
  getTourplanList(registrationValue) {
    debugger;
    this.sIsLoading = 'loading-data';
    var D_data = {
      "MRId": this._StdTourplanService.myFilterform.get("MrId").value || "0",
  ...
  NewTourplan() {
    ...
    var m_data = {
      "MRId": "Shuhas",//this._StdTourplanService.myFilterform.get("MrId").value.MrName
        
    }

    const dialogRef = this._matDialog.open(AddStdtourplanComponent,
      {
        maxWidth: '80%',
        height: '69%', 
        width: '100%',
    ...
    dialogRef.afterClosed().subscribe(result => {
      console.log(result)
      this._StdTourplanService.getTourplaneDetails(this.dData1).subscribe(Visit => {
        this.dataArray = Visit;
        console.log(this.dataArray)
        this.sIsLoading = '';
      },
      ...
      // this.getTourplanList();
    });
  }

  
onClear(){
  this._StdTourplanService.myFilterform.reset(
    {
      start: [],
      end: []
    }
...
console.log(row);
  ...
  console.log(m_data);
  this._StdTourplanService.populateFormpersonal(m_data);
